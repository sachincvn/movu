...
class DrmManager {
    fun buildDrmSessionManager(
        ...
        return when (drmScheme.lowercase()) {
            ...
            "clearkey" -> {
                ...
                val clearKeyLicenseUri = createClearKeyLicenseUri(keys)
    ...
    private fun createClearKeyLicenseUri(licenseKeys: List<String>): String {
        ...
        for (key in licenseKeys) {
            ...
            if (keyParts.size == 2) {
                try {
                    ...
                    android.util.Log.d("DrmManager", "Added ClearKey: ${keyParts[0]} -> ${keyParts[1]}")
                } catch (e: Exception) {
                    android.util.Log.e("DrmManager", "Failed to parse ClearKey: $key", e)
                }
            } else {
                android.util.Log.w("DrmManager", "Invalid ClearKey format: $key (expected keyId:keyValue)")
            }
        }

        if (keysList.isEmpty()) {
            android.util.Log.e("DrmManager", "No valid ClearKey pairs found")
            return ""
        }
        ...
        android.util.Log.d("DrmManager", "ClearKey license JSON: $jsonData")
